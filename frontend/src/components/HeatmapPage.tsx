...
function getMoodTextColor(mood: number | null): string {
...
export default function HeatmapPage() {
  const [data, setData] = useState<Record<string, { avg_mood: number | null, count: number }>>({})
  const [summary, setSummary] = useState<{ avg_mood: number | null, count: number } | null>(null)
    ...
    <div style={{ padding: "24px 20px" }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: "24px", fontWeight: 700, color: "#1a1a1a" }}>
              {summary.avg_mood !== null ? summary.avg_mood : "N/A"}
              {summary.avg_mood !== null && <span style={{ fontSize: "14px", color: "#888", fontWeight: 400 }}> / 10</span>}
            </p>
          </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", marginBottom: "24px" }}>
        {COLLEGES.map(college => {
          ...
          const mood = collegeData?.avg_mood ?? null
          ...
          return (
            ...
            >
              ...
              <p style={{ fontSize: "20px", fontWeight: 700, color: getMoodTextColor(mood) }}>
                {mood !== null ? mood : "--"}
              </p>
              <p style={{ fontSize: "11px", color: getMoodTextColor(mood) }}>
                {getMoodLabel(mood)}
              </p>
            </button>
          )
      ...
      {selected && selectedData && (
        <div style={{
          ...
          {selectedData.avg_mood !== null ? (
            <>
              <p style={{ fontSize: "32px", fontWeight: 700, color: getMoodTextColor(selectedData.avg_mood), marginBottom: "4px" }}>
                {selectedData.avg_mood} / 10
              </p>
              <p style={{ fontSize: "14px", color: "#888", marginBottom: "12px" }}>
                {getMoodLabel(selectedData.avg_mood)}
              </p>
              <p style={{ fontSize: "13px", color: "#aaa" }}>
                Based on {selectedData.count} check-in{selectedData.count !== 1 ? "s" : ""} in the last 24 hours
              </p>
            </>
          ) : (
            <div>
              <p style={{ fontSize: "15px", color: "#888", marginBottom: "8px" }}>
                Not enough data yet for {selectedCollege?.label}.
              </p>
              <p style={{ fontSize: "13px", color: "#aaa" }}>
                At least 10 check-ins are needed to show data for a college. Currently {selectedData.count || 0} check-in{selectedData.count !== 1 ? "s" : ""}.
              </p>
            </div>
          )}
        </div>
