...
const sections = [
  ...
  { title: "Peer and supporter information", body: "Peer Connect remains disabled during safety review. If it is later enabled, public supporter profiles use an immutable ID and limited profile fields; they do not include private phone numbers, email addresses, references, credentials, or request content. Private supporter, requester, reference, connection, and report data is stored in encrypted server payloads and is available only to authorized roles. Audit records use IDs and allowlisted operational metadata rather than contact details or user-written content." },
  { title: "Retention and deletion limits", body: "Administrator sessions and peer access tokens expire after 15 minutes by default. The peer backend assigns default retention windows of 90 days to requester and connection records and 365 days to supporter, report, audit, and status-history records. Actual erasure depends on the operator scheduling the protected retention purge. Withdrawal and deletion paths erase private peer payloads, but database deletion does not automatically erase provider logs or backup copies, which follow separate operator and provider schedules." },
  { title: "Third parties", body: "The deployment uses a hosting environment and PostgreSQL-compatible database. If peer email notifications are configured through Resend, the redesigned templates send only a fixed message and server-generated record ID, not contact details, request messages, or report reasons. The current application has no active Redis integration. Following phone numbers or external links sends data directly to those outside services under their own practices. CornellPulse does not currently include a third-party browser analytics SDK." },
]
