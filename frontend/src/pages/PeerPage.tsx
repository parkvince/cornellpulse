    ...
    <div style={{ backgroundColor: "#fff", border: "1px solid #e5e5e5", borderRadius: "12px", padding: "18px", marginBottom: "12px" }}>
      ...
      {supporter.interests && supporter.interests.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "12px" }}>
          {supporter.interests.slice(0, 4).map(i => (
      ...
      <button
        onClick={() => onRequest(supporter)}
        style={{ width: "100%", padding: "12px", backgroundColor: "#1a1a1a", color: "#fff", border: "none", borderRadius: "8px", fontSize: "14px", fontWeight: 600, cursor: "pointer" }}
      >
        Ask to meet up
      </button>
    </div>
        ...
        <div style={{ backgroundColor: "#fff", borderRadius: "16px 16px 0 0", padding: "32px 24px 48px", width: "100%" }}>
          <div style={{ textAlign: "center", marginBottom: "20px" }}>
            ...
            <p style={{ fontSize: "14px", color: "#666", lineHeight: 1.6 }}>
              We will reach out to you within 24 hours to connect you with {supporter.name}. Check your email.
            </p>
          </div>
          <button onClick={onSubmit} style={{ width: "100%", padding: "14px", backgroundColor: "#1a1a1a", color: "#fff", border: "none", borderRadius: "10px", fontSize: "15px", fontWeight: 600, cursor: "pointer" }}>
            Done
          </button>
        </div>
      </div>
    ...
    <div style={{ position: "fixed", top: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: "430px", height: "100%", backgroundColor: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", alignItems: "flex-end" }}>
      <div style={{ backgroundColor: "#fff", borderRadius: "16px 16px 0 0", padding: "24px 24px 48px", width: "100%", maxHeight: "85vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
          ...
          <button onClick={onClose} style={{ fontSize: "20px", color: "#aaa", cursor: "pointer", backgroundColor: "transparent", border: "none", lineHeight: 1 }}>x</button>
        </div>

        <div style={{ marginBottom: "14px" }}>
          <label style={{ fontSize: "13px", fontWeight: 600, color: "#333", display: "block", marginBottom: "6px" }}>Your name <span style={{ color: "#c00" }}>*</span></label>
          <input value={form.requester_name} onChange={e => update("requester_name", e.target.value)} placeholder="Your first name is fine" style={{ width: "100%", padding: "11px 14px", border: "1px solid #e5e5e5", borderRadius: "10px", fontSize: "15px", backgroundColor: "#fff" }} />
        </div>

        <div style={{ marginBottom: "14px" }}>
          <label style={{ fontSize: "13px", fontWeight: 600, color: "#333", display: "block", marginBottom: "6px" }}>Your Cornell email <span style={{ color: "#c00" }}>*</span></label>
          <input value={form.requester_email} onChange={e => update("requester_email", e.target.value)} placeholder="netid@cornell.edu" type="email" style={{ width: "100%", padding: "11px 14px", border: "1px solid #e5e5e5", borderRadius: "10px", fontSize: "15px", backgroundColor: "#fff" }} />
        </div>

        <div style={{ marginBottom: "14px" }}>
          <label style={{ fontSize: "13px", fontWeight: 600, color: "#333", display: "block", marginBottom: "6px" }}>Your phone <span style={{ color: "#999", fontWeight: 400 }}>(optional)</span></label>
          <input value={form.requester_phone} onChange={e => update("requester_phone", e.target.value)} placeholder="So we can text you" type="tel" style={{ width: "100%", padding: "11px 14px", border: "1px solid #e5e5e5", borderRadius: "10px", fontSize: "15px", backgroundColor: "#fff" }} />
        </div>
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <textarea value={form.message} onChange={e => update("message", e.target.value)} maxLength={300} placeholder="Whatever feels right to share. Completely up to you." rows={3} style={{ width: "100%", padding: "11px 14px", border: "1px solid #e5e5e5", borderRadius: "10px", fontSize: "14px", backgroundColor: "#fff", resize: "none" }} />
        </div>
        ...
        <div style={{ backgroundColor: "#f9f9f9", borderRadius: "10px", padding: "12px", marginBottom: "20px" }}>
          <p style={{ fontSize: "12px", color: "#888", lineHeight: 1.6 }}>
            We will introduce you both over email within 24 hours. {supporter.name} has been vetted and approved by our team.
          </p>
        </div>
        ...
        <button
          ...
          style={{ width: "100%", padding: "14px", backgroundColor: canSubmit && !loading ? "#1a1a1a" : "#ccc", color: "#fff", border: "none", borderRadius: "10px", fontSize: "15px", fontWeight: 600, cursor: canSubmit && !loading ? "pointer" : "default" }}
        >
...
export default function PeerPage() {
  ...
  const filtered = supporters.filter(s => {
    const matchAvail = !availabilityFilter || (s.availability || []).includes(availabilityFilter)
    const matchInterest = !interestFilter || (s.interests || []).includes(interestFilter)
    return matchAvail && matchInterest
  })
  ...
  return (
    <div style={{ padding: "24px 20px 0" }}>
      ...
      <p style={{ fontSize: "14px", color: "#888", marginBottom: "24px", lineHeight: 1.5 }}>
        Connect with a Cornell student who wants to listen, grab food, or just be there.
      </p>
        ...
        <div>
          ...
          {loading && (
            <div style={{ textAlign: "center", padding: "48px 0" }}>
              <p style={{ fontSize: "14px", color: "#aaa" }}>Loading supporters...</p>
            </div>
          )}
          ...
          {!loading && filtered.length === 0 && (
            <div style={{ backgroundColor: "#fff", border: "1px solid #e5e5e5", borderRadius: "12px", padding: "32px 20px", textAlign: "center" }}>
              ...
              <p style={{ fontSize: "14px", color: "#888", lineHeight: 1.6, marginBottom: "20px" }}>
                We are building our network. Be the first to sign up as a peer supporter and help other Cornell students.
              </p>
              <button onClick={() => setTab("signup")} style={{ padding: "12px 24px", backgroundColor: "#1a1a1a", color: "#fff", border: "none", borderRadius: "10px", fontSize: "14px", fontWeight: 600, cursor: "pointer" }}>
          ...
          <div style={{ backgroundColor: "#f9f9f9", border: "1px solid #e5e5e5", borderRadius: "12px", padding: "16px", marginTop: "8px", marginBottom: "24px" }}>
            <p style={{ fontSize: "13px", color: "#666", lineHeight: 1.6 }}>
              All peer supporters have been vetted and approved by the CornellPulse team. If you are in crisis please call 988 or Cornell Health at 607-255-5155.
            </p>
          </div>
        </div>
      )}
  ...
  if (submitted) {
    return (
      <div style={{ padding: "48px 0", textAlign: "center" }}>
        ...
        <p style={{ fontSize: "14px", color: "#666", lineHeight: 1.6 }}>
          We will review your application and reach out within a few days. We really appreciate you being willing to show up for other students.
        </p>
      </div>
    )
  }
  ...
  return (
    <div style={{ paddingBottom: "40px" }}>
      <p style={{ fontSize: "14px", color: "#666", lineHeight: 1.6, marginBottom: "28px" }}>
        Sign up to be someone that students can reach out to when they just want to talk, grab food, or not be alone. All applications are reviewed before approval.
      </p>
        ...
        {[
          ...
          <div key={f.field} style={{ marginBottom: "14px" }}>
            ...
            <input
              ...
              type={f.type}
              style={{ width: "100%", padding: "12px 14px", border: "1px solid #e5e5e5", borderRadius: "10px", fontSize: "15px", backgroundColor: "#fff", color: "#1a1a1a" }}
            />
          </div>
        ))}
        <div style={{ marginBottom: "14px" }}>
      ...
      <section style={{ marginBottom: "28px" }}>
        ...
        <p style={{ fontSize: "13px", color: "#aaa", marginBottom: "12px" }}>A sentence or two. This is what students see when they are deciding who to reach out to.</p>
        <textarea value={form.about} onChange={e => update("about", e.target.value)} maxLength={300} placeholder="e.g. Junior in CS, originally from NYC. I know what it feels like to be overwhelmed at Cornell and I am always down to grab food or just talk." rows={4} style={{ width: "100%", padding: "12px 14px", border: "1px solid #e5e5e5", borderRadius: "10px", fontSize: "14px", backgroundColor: "#fff", resize: "none" }} />
      ...
      <section style={{ marginBottom: "28px", backgroundColor: "#f9f9f9", border: "1px solid #e5e5e5", borderRadius: "12px", padding: "20px" }}>
        ...
        <p style={{ fontSize: "13px", color: "#666", lineHeight: 1.6, marginBottom: "20px" }}>
          We require one reference who can speak to your character. We will contact them before approving your application.
        </p>
        {[
          ...
          <div key={f.field} style={{ marginBottom: "14px" }}>
            ...
            <input
              ...
              placeholder={f.placeholder}
              type={f.type}
              style={{ width: "100%", padding: "12px 14px", border: "1px solid #e5e5e5", borderRadius: "10px", fontSize: "15px", backgroundColor: "#fff" }}
            />
          </div>
        ))}
      </section>
      ...
      <div style={{ backgroundColor: "#f9f9f9", border: "1px solid #e5e5e5", borderRadius: "12px", padding: "14px", marginBottom: "20px" }}>
        <p style={{ fontSize: "12px", color: "#888", lineHeight: 1.6 }}>By submitting you agree that CornellPulse may contact you and your reference. Your info will never be shared publicly. You can opt out at any time.</p>
      </div>
