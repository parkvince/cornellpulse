...
test("onboarding, navigation, emergency dialog, resources, privacy, Peer sandbox, and disabled routes", async ({ page }) => {
  ...
  await peerTab.click()
  await expect(page.getByRole("heading", { name: "Need someone to talk to?" })).toBeVisible()
...
test("Peer sandbox creates a real requester and submits a server-backed connection request", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Talk to Open Sandbox Peer" }).click()
  await page.getByLabel("First name or nickname").fill("Requester One")
