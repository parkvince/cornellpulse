...
The in-app browser was set to 320×568, 375×667, 393×852, 412×915, 768×1024, and 1280×900. At every size, Home, Resources, History & Privacy, Privacy & Data, disabled Peer, disabled Admin, and the custom 404 were opened. All 42 route/viewport combinations had the expected heading, a visible Immediate help control, and no document-level horizontal overflow.
...
| History, export, deletion, reminders | Saved-plan controls, local trend, reminders, follow-up, retention selector, export, and delete/clear controls were present. The complete export/delete lifecycle is also exercised in Playwright. |
...
Network interception and browser-context offline mode are more deterministic than the in-app browser for these states. The 42-test Playwright matrix therefore supplies the evidence for offline reload, aggregate endpoint failure, retained local results, axe checks, and install metadata. The backend suite supplies unauthorized, rate-limited, maintenance, outage, notification-failure, spoofing, injection, duplicate, retention, and readiness evidence.
