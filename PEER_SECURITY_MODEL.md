...
2. Apply `backend/migrations/20260802_peer_security_redesign.sql` and then `backend/migrations/20260802_supporter_onboarding.sql` in staging. They add and backfill columns/tables without removing existing rows or columns. Only unique legacy supporter names are linked automatically; ambiguous names remain null for human reconciliation. Legacy reference data does not satisfy the new consent gate.
